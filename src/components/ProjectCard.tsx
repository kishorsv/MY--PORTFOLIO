...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelect }) => {
  return (
    ...
    >
      {/* Image container */}
      <div className="relative h-48 sm:h-56 w-full overflow-hidden bg-neutral-950/50">
        <img
          src={project.image}
          alt={project.title}
        ...
        <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between pointer-events-none">
          <span className="px-3 py-1 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-white/10 backdrop-blur-md text-indigo-300 border border-white/10 shadow-sm">
            {project.category}
          </span>
          {project.featured && (
      ...
      <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
        ...
        {/* Buttons / Actions */}
        <div className="flex items-center justify-between pt-5 mt-5 border-t border-white/10" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => onSelect(project)}
          ...
          <div className="flex items-center gap-2">
            ...
            {project.liveUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="p-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white transition-colors border border-indigo-500/30"
